...
// Input schema for Create Integration Capabilities Big Segment Store
...
// Output schema for Create Integration Capabilities Big Segment Store
const outputSchema = {
  type: "object",
  properties: {
    _links: {
...
export default {
  name: "Create Integration Capabilities Big Segment Store",
  description:
    "Creates create integration capabilities big segment store in LaunchDarkly",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input: EventInput) => {
        const { projectKey, environmentKey, integrationKey, ...inputData } =
          input.event.inputConfig;
        const endpoint = `/api/v2/integration-capabilities/big-segment-store/${projectKey}/${environmentKey}/${integrationKey}`;
