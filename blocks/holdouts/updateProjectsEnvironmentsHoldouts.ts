...
// Input schema for Update Projects Environments Holdouts
...
// Output schema for Update Projects Environments Holdouts
const outputSchema = {
  type: "object",
  properties: {
    _id: {
    ...
    status: {
      type: "string",
      enum: ["created", "enabled", "running", "ended"],
    },
    description: {
      type: "string",
    },
  ...
  required: ["_id", "status", "holdoutAmount", "createdAt", "baseExperiment"],
};

export default {
  name: "Update Projects Environments Holdouts",
  description: "Updates update projects environments holdouts in LaunchDarkly",
  category: "Holdouts",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input: EventInput) => {
        const { projectKey, environmentKey, holdoutKey, ...inputData } =
          input.event.inputConfig;
        const endpoint = `/api/v2/projects/${projectKey}/environments/${environmentKey}/holdouts/${holdoutKey}`;
